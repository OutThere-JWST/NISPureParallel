...
from astropy.io.ascii import Tab
...
if __name__ == '__main__':
    ...
    # Create RGB file
    shutil.copy(path.join(plots,f'{fname}.rgb.png'),path.join(fitsmap,f'RGB.png'))
    files.append(path.join(fitsmap,f'RGB.png'))
    ...
    for f in filts:
        ...
        shutil.copy(
            path.join(prep,f'{fname}-{f.lower()}n-clear_drc_sci.fits'),
            outfile
        )
    ...
    for g in grisms:
        ...
        pre = f'{fname}-{g.lower()}-'
        pas = [f for f in glob(path.join(extract,f'{pre}*grism_sci*')) if 'proj' not in f]
    ...
    if path.exists(results_file):
        ...
        for i in results['id']:
            ...
            imgs = [f'{fname}_{str(i).zfill(5)}.{suffix}.png' for suffix in ['stack','line','full']]
            allims.extend(imgs)
            
            # Append to list of extracted links
            links.append(
                '<br>'.join([
                    f'<img src="spectra/{f}" width="500">'
                for f in imgs])
            )
    ...
    for f in allims:
        shutil.copy(
            path.join(extract,f),
            path.join(fitsmap,'map','spectra')
        )

    # FitsMap Add Ons
    with open(path.join(fitsmap,'map/js/index.js'),'r') as f: js = f.readlines()
    ...
    for i,line in enumerate(js):
        ...
        if 'layerControl' in line:
            ...
            for j,m in enumerate(menuitems):
                if '_' not in m: continue # Ignoreif we don't have to do anything
    ...
    js = js[:i]+["const settingsControl = L.control.settings({\n",
        "    position: 'topleft',\n",
        "    // catalogs:catalogs,\n",
        "}).addTo(map);\n",
        "\n"
    ...
    with open(path.join(fitsmap,'map/js/index.js'),'w+') as f: f.write(''.join(js))
    with open(path.join(fitsmap,'map/index.html'),'r') as f: html = f.readlines()
    i = html.index("    <link rel='preload' href='css/TileNearestNeighbor.min.css'  as='style' onload='this.rel=\"stylesheet\"'/>\n")
    html.insert(i+1,'    <link href="css/SettingsControl.css" rel="stylesheet" type="text/css"/>\n')
    i = html.index("    <script defer src='js/index.js'></script>\n")
    html.insert(i,"    <script defer src='js/settingsControl.js'></script>\n")
    with open(path.join(fitsmap,'map/index.html'),'w+') as f: f.write(''.join(html))
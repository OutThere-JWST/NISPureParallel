...
def measure_traces_one_exposure(
    ...
    for row in cat_sel:
        ...
        if order != '0':
            ...
            edge_start_flipped = find_step_derivative(
                np.fliplr(work)[:, :50], bg_npix=10
            )
            ...
            try:
                for k_test in [5, 4, 3]:
                    contour, level, (py, px), (bg, noise) = find_contour_around_peak(
                        work, smooth_sigma=0.8, k=k_test
                    )
                ...
                if summary is None:
                    print(
                        f'  [WARN] Object {obj_id}: no valid contour found → skipping'
                    )
                    continue
